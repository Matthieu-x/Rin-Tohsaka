...
const colorDeNumero = numero => {
    if (numero === 0) return 'verde'
    return NUMEROS_ROJOS.includes(numero) ? 'rojo' : 'negro'
}

const TEXTOS_RULETA_GANA = [
    'La bolita cayó justo donde apostaste',
    'La ruleta te sonrió esta vez',
    'Tiro perfecto en la mesa',
    'La casa no pudo contra vos esta ronda',
    'La bolita rebotó dos veces y aun así cayó a tu favor'
]

const TEXTOS_RULETA_PIERDE = [
    'La bolita se fue para cualquier otro lado',
    'Estuviste cerca, pero la casa se quedó con todo',
    'La ruleta no te acompañó hoy',
    'Mala tirada, probá de nuevo',
    'La bolita tenía otros planes'
]

// tipo: 'color' (valor: 'rojo'/'negro'), 'numero' (valor: 0-36), 'paridad' (valor: 'par'/'impar')
export const ejecutarRuleta = (jid, tipo, valor, monto) => {
    const numeroSalido = numeroEntreRango(0, 36)
    const colorSalido = colorDeNumero(numeroSalido)

    let gano = false
    let multiplicador = 0

    if (tipo === 'color' && valor === colorSalido && colorSalido !== 'verde') {
        gano = true
        multiplicador = 2
    } else if (tipo === 'numero' && Number(valor) === numeroSalido) {
        gano = true
        multiplicador = 36
    } else if (tipo === 'paridad' && numeroSalido !== 0) {
        const esPar = numeroSalido % 2 === 0
        if ((valor === 'par' && esPar) || (valor === 'impar' && !esPar)) {
            gano = true
            multiplicador = 2
        }
    }

    const ganancia = gano ? Math.floor(monto * multiplicador) : 0
    ...
    return {
        ...
        ganancia,
        monto,
        texto: elegirAleatorio(gano ? TEXTOS_RULETA_GANA : TEXTOS_RULETA_PIERDE)
    }
}
