...
import { obtenerRutaFotoIdentidad } from './identidad.js'
...
const obtenerFotoParaMensaje = async (conn, jid) => {
    try {
        const url = await conn.profilePictureUrl(jid, 'image')
        if (url) return { url }
    } catch (e) {}

    const rutaPersonalizada = obtenerRutaFotoIdentidad(conn)
    if (rutaPersonalizada) {
        try {
            return { buffer: readFileSync(rutaPersonalizada) }
        } catch (e) {}
    }

    if (existsSync(RUTA_FOTO_DEFAULT)) {
        try {
            return { buffer: readFileSync(RUTA_FOTO_DEFAULT) }
        } catch (e) {}
    }
    ...
    conn.ev.on('group-participants.update', async function enviarBienvenidaDespedida({ id, participants, action }) {
        try {
            ...
            for (const participante of participants) {
                ...
                const foto = await obtenerFotoParaMensaje(conn, participantJid)

                const caption = `${encabezado}\n\n> ${texto}`

                const contenido = foto
                    ? (foto.url
                        ? { image: { url: foto.url }, caption, mentions: [participantJid] }
                        : { image: foto.buffer, caption, mentions: [participantJid] })
