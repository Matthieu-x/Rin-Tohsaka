...
const URL_FONDO_BIENVENIDA = 'https://i.ibb.co/Y4zcX7fr/file-00000000271081f5825ba8a8ebc6a680.png'
const URL_FONDO_DESPEDIDA = 'https://i.ibb.co/C3J6S7MC/file-000000009b3481f5a7f2d0be9a7005c7.png'
...
const cacheFondos = {
    bienvenida: { imagen: null, timestamp: 0 },
    despedida: { imagen: null, timestamp: 0 }
}

const FONDO_CACHE_MS = 60 * 60 * 1000

const obtenerFondo = async (tipo = 'bienvenida') => {
    const ahora = Date.now()
    const entrada = cacheFondos[tipo]

    if (entrada.imagen && (ahora - entrada.timestamp) < FONDO_CACHE_MS) {
        return entrada.imagen
    }

    const url = tipo === 'despedida' ? URL_FONDO_DESPEDIDA : URL_FONDO_BIENVENIDA

    try {
        console.log(`[WELCOME] Descargando fondo de ${tipo}...`)
        const res = await fetch(url)
        const buffer = Buffer.from(await res.arrayBuffer())
        entrada.imagen = await loadImage(buffer)
        entrada.timestamp = ahora
        console.log(`[WELCOME] Fondo de ${tipo} cargado`)
        return entrada.imagen
    } catch (error) {
        console.error(`[WELCOME] Error cargando fondo de ${tipo}:`, error.message)
        throw error
    }
}
...
const generarCarta = async (fotoUsuarioUrl, nombreUsuario, tipo = 'add') => {
    ...
    const esDespedida = tipo === 'remove'
    const fondo = await obtenerFondo(esDespedida ? 'despedida' : 'bienvenida')
    ...
    const subtitulo = esDespedida ? '¡Hasta pronto!' : '¡Bienvenido/a!'
        ...
        try {
            ...
            for (const participante of participants) {
                ...
                try {
                    ...
                    const urlFallback = action === 'remove' ? URL_FONDO_DESPEDIDA : URL_FONDO_BIENVENIDA

                    await conn.sendMessage(
                        chatId,
                        {
                            image: { url: urlFallback },
                            caption: caption,
                            mentions: [participantJid]
                        }
                    ).catch(_ => null)
